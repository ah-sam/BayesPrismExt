#' function to extract posterior mean of cell type / cell state fraction matrix
...
#' function to extract posterior CV of cell type / cell state fraction matrix
#' @param bp a BayesPrism object
#' @param which.theta a character variable to denote to extract first or final Gibbs 
#' @param state.or.type a character variable to denote if cell type or cell state 
#' @return a N*K matrix 
get.fraction.cv <- function(bp,
                            which.theta,
                            state.or.type){
    ...
    if(which.theta=="first" & state.or.type=="state")
        return(bp@posterior.initial.cellState@theta.cv) 
    if(which.theta=="first" & state.or.type=="type")
        return(bp@posterior.initial.cellType@theta.cv)  
    if(which.theta=="final"){
        if(state.or.type=="state")
            warning("Warning: only cell type is available for updated Gibbs. Returning cell type info.")
...
#' function to extract posterior mean of sample specific gene expression profile for the given cell type
#' @param bp a BayesPrism object
#' @param state.or.type a character variable to denote if cell type or cell state 
...
#' function to extract posterior CV of sample specific gene expression profile for the given cell type
#' @param bp a BayesPrism object
#' @param state.or.type a character variable to denote if cell type or cell state 
#' @param cell.name a character variable to denote name of cell type/state 
#' @return a N*G matrix for the cell type/state of interest
get.exp.cv <- function(bp,
                       state.or.type,
                       cell.name){
    ...
    if(state.or.type=="state")
        return(bp@posterior.initial.cellState@Z.cv[,,cell.name])
    
    if(state.or.type=="type")
        return(bp@posterior.initial.cellType@Z.cv[,,cell.name])
